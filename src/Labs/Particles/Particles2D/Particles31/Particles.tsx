    ...
    for (let i = 0; i < this.bundlesNumber; i++) {
      this.bundles.push(
        new Bundle(
          this.stageWidth,
          this.stageHeight,
          getRandom(1, getRandom(10, getRandom(20, 200)))
        )
      );
    }
  }
...
class Bundle {
  ...
  constructor(stageWidth: any, stageHeight: any, particleNumber: any) {
    ...
    this.opacity = getRandom(0, 0.1);
    this.opacitySpeed = getRandom(0.01, getRandom(0.01, 0.1));
    this.time = 0;
  }
  ...
  draw(ctx: any, pos: any) {
    this.time++;
    const opacity = Math.sin(this.time * this.opacitySpeed) * this.opacity;
    this.particles.map((particle: any) => {
      particle.draw(ctx, opacity, pos, this.stageWidth, this.stageHeight);
    });
  }
}
...
class Particle {
  ...
  constructor(
    ...
    this.bundleCenter = {
      x:
        bundleCenter.x +
        (Math.random() < 0.5 ? -1 : 1) * getRandom(0, getRandom(0, 20)),
      y:
        bundleCenter.y +
        (Math.random() < 0.5 ? -1 : 1) * getRandom(0, getRandom(0, 20)),
    };
  ...
  draw(ctx: any, opacity: any, pos: any, stageWidth: any, stageHeight: any) {
    this.pos.x =
      pos.x +
