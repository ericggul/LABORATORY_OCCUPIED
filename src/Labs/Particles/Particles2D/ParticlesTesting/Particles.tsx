  ...
  return (
    <>
      ...
      <div className={style.compWrapper}>
        {new Array(2500).fill(0).map((e, j) => (
          <div className={style.component} key={100 + j}>
            {new Array(4).fill(0).map((e, i) => (
              <div className={style.cover} key={i} />
            ))}
          </div>
        ))}
      </div>
...
class Particle {
  ...
  draw(ctx: any, opacity: any, pos: any) {
    ...
    this.pos.x += ((pos.x - currentPos.x) / 20) * randomness;

    this.pos.y += ((pos.y - currentPos.y) / 20) * randomness;
