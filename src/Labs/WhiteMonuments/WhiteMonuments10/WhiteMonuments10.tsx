import style from "./WhiteMonuments10.module.scss";
import { useState, useEffect, useCallback } from "react";

export default function WhiteMonuments10() {
  ...
  const Container = ({ i, loc, hanger }: Props) => {
    ...
    const [width, setWidth] = useState(getRandom(1, 10));
    const [height, setHeight] = useState(getRandom(1, 100));
  ...
  return (
    <div className={style.whole}>
      {loading &&
        size.height > 1 &&
        new Array(Math.ceil(size.height / 10 + 6)).fill(0).map((e, j) => {
          const hanger = getRandom(10, 50);
          return new Array(200)
            .fill(0)
            .map((e, i) => (
              <Container i={i} loc={(j - 3) * 10} hanger={hanger} key={i} />
            ));
        })}
    </div>
  );
