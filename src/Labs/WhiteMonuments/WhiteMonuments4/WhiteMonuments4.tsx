import style from "./WhiteMonuments4.module.scss";
import { useState, useEffect, useCallback } from "react";

export default function WhiteMonuments4() {
  ...
  const Container = ({ i, loc }: Props) => {
    const [color, setColor] = useState(getColor());
    const [width, setWidth] = useState(200);
    const [height, setHeight] = useState(200);
    const [top, setTop] = useState(loc + 180 * Math.sin((i / 180) * Math.PI));
    const [left, setLeft] = useState(
      ((size.width + 200) / 2) * (i / 180) - 100
    );
    return (
      <div
  ...
  return (
    <div className={style.whole}>
      {loading &&
        size.height > 1 &&
        new Array(Math.ceil(size.height / 100 + 6))
          .fill(0)
          .map((e, j) =>
            new Array(360)
              .fill(0)
              .map((e, i) => <Container i={i} loc={(j - 3) * 200} key={i} />)
          )}
    </div>
  );
}
