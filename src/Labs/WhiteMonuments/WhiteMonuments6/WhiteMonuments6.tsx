import style from "./WhiteMonuments6.module.scss";
import { useState, useEffect, useCallback } from "react";

export default function WhiteMonuments6() {
  ...
  return (
    <div className={style.whole}>
      {loading &&
        size.height > 1 &&
        new Array(Math.ceil(size.height / 100 + 6)).fill(0).map((e, j) => {
          const hanger = getRandom(10, 50);
          return new Array(540)
            .fill(0)
            .map((e, i) => (
              <Container i={i} loc={(j - 3) * 230} hanger={hanger} key={i} />
            ));
        })}
    </div>
  );
