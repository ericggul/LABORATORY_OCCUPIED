...
class App {
  ...
  init() {
    const array = [0.2, 0.4, 0.6, 0.8];

    const startPoints = [
      { x: this.stageWidth * 0.1, y: this.stageHeight * 0.1 },
      { x: this.stageWidth * 0.1, y: this.stageHeight * 0.9 },
      { x: this.stageWidth * 0.9, y: this.stageHeight * 0.1 },
      { x: this.stageWidth * 0.9, y: this.stageHeight * 0.9 },
    ];
    for (var i = 0; i < this.particlesNum; i++) {
      this.particles.push(
        new Plane(
          ...
          startPoints[Math.floor(Math.random() * 4)],
          this.ctx
        )
      );
    }
...
class Plane {
  ...
  constructor(x: any, y: any, start: any, ctx: any) {
    this.x = x;
    this.y = y;
    this.ctx = ctx;
    this.start = start;
