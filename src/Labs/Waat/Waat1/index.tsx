...
class App {
    ...
    for (let i = 0; i < this.circleNumber; i++) {
      this.circles.push(
        new Circle(
          ...
          10,
          getRandom(Math.PI * 1.5, Math.PI * 2)
        )
      );
    }
  ...
  draw() {
    this.circles.forEach((circle: any) => circle.draw(this.ctx));
  }
}
...
class Circle {
  ...
  draw(ctx: any) {
    ctx.save();
    ctx.fillStyle = `rgba(${getRandom(0, 250)}, ${getRandom(
      110,
      150
    )}, ${getRandom(150, 250)}, 0.005)`;
