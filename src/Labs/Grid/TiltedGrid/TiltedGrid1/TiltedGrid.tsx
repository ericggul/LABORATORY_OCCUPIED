      ...
      this.gridSets.push(
        new GridSet(
          ...
          20,
          {
            h: getRandom(170, 180),
            s: getRandom(40, 60),
            l: 50 + (i * 30) / this.gridSetsNum + getRandom(0, 20),
          },
          this.tiltedAngle
        )
      );
...
class GridSet {
  ...
  constructor(
    ...
    this.elements = [];
    this.widthArray = boundedArrayGenerator(width / 6, width, elementsNum);
    this.heightArray = boundedArrayGenerator(height / 6, height, elementsNum);
    ...
    for (let i = 0; i < this.elementsNum; i++) {
      this.elements.push(
        new Element(
          ...
          this.heightArray[i],
          this.angle,
          `hsl(${this.color.h}, ${this.color.s * getRandom(0.95, 1.05)}%, ${
            this.color.l * getRandom(0.9, 1.1)
          }%)`
        )
      );
    }
...
class Element {
  ...
  constructor(x: any, y: any, width: any, height: any, angle: any, color: any) {
    ...
    this.speedIndex = getRandom(0.02, 0.06);
    this.time = 0;
    this.interval = getRandom(300, 1000);
  }
