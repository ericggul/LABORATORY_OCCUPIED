import style from "./GridColor5.module.scss";
import { useState, useEffect, useCallback } from "react";

export default function GridColor5() {
  ...
  const Container = () => {
    ...
    const adjuster = (i: number) => {
      ...
      const getRandomColorEl = (i: number, j: number) => {
        return `rgb(${250 - i * 2 - j * 2}, ${x + 100}, ${180 + i + j ** 0.2})`;
      };

      return `radial-gradient(${getRandomColorEl(
        Math.abs(x - elements.x),
        Math.abs(y - elements.y)
      )} , white)`;
    };
