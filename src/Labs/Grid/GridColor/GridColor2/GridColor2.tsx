import style from "./GridColor2.module.scss";
import { useState, useEffect, useCallback } from "react";

export default function GridColor2() {
  ...
  const Container = () => {
    ...
    const adjuster = (i: number) => {
      return i > (heightSize * 2) / 3
        ? ((2 * i - heightSize) / heightSize) * 160
        : ((i * 0.1) / heightSize) * 160;
    };
        ...
        {new Array(heightSize * 100).fill(0).map((e, i) => {
          return (
            <div
              className={style.box}
              style={{
                background: getColor(adjuster(Math.floor(i / 100))),
              }}
              key={i}
            />
          );
