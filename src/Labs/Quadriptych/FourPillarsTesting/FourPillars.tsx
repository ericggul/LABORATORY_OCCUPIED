...
class App {
  ...
  resize() {
    ...
    this.squareHeight = this.stageHeight / 10;
    this.ctx.scale(1, 1);
    this.init();
  }
  ...
  init() {
    for (
      ...
      i < this.stageWidth + this.squareWidth;
      i += getRandom(this.squareWidth / 40, this.squareWidth / 20)
    ) {
      for (
        let j = -this.squareHeight;
        j < this.stageHeight + this.squareHeight;
        j += this.squareHeight
      ) {
        this.squareSets.push(
          new Square(i, j, this.squareWidth, this.squareHeight, this.ctx)
        );
      }
    }
  }
...
class Square {
  ...
  constructor(x: any, y: any, width: any, height: any, ctx: any) {
    ...
    this.scratchSets = [];
    this.scratchNumber = getRandom(200, 1000);
