...
class App {
  ...
  resize() {
    ...
    this.circleNum = (this.stageWidth + this.stageHeight) / 10;
    ...
    for (let i = 1; i < this.circleNum; i++) {
      const radiusAdd =
        getRandom(0, 0.001) * (this.stageWidth + this.stageHeight);
      const theta = getRandom(0, Math.PI * 0.1);
      this.xArray.push(
        this.xArray[i - 1] + radiusAdd * getRandom(0, 1) * Math.cos(theta)
      );
      this.yArray.push(
        this.yArray[i - 1] + radiusAdd * getRandom(0, 1) * Math.sin(theta)
      );
      this.radiusArray.push(this.radiusArray[i - 1] + radiusAdd);
    }
